...
import evaluate
import numpy as np
from datasets import Value, Dataset, DatasetDict, load_dataset, concatenate_datasets
...
class DataTrainingArguments:
    ...
    dataset_config_names: Optional[List[str]] = field(
...
def load_multiple_datasets(
    ...
    **kwargs,
    ...
    for i, dataset_name in enumerate(dataset_names):
        try:
            dataset = load_dataset(dataset_name, dataset_config_names[i], split=split, **kwargs)
        except ValueError:
            continue
        if "tweet" in dataset.features:
            dataset = dataset.rename_column("tweet", "text")
        dataset = dataset.remove_columns(set(dataset.features.keys()) - set(["text", "label"]))
        all_datasets.append(dataset)

    return concatenate_datasets(all_datasets)


def load_multiple_test_datasets(dataset_names: List[str], dataset_config_names: List[str], **kwargs) -> List[Dataset]:
    all_datasets, dataset_names = [], []
    for i, dataset_name in enumerate(dataset_names):
        try:
            dataset = load_dataset(dataset_name, dataset_config_names[i], split="test", **kwargs)
        except ValueError:
            print(dataset_name)
            continue
        if "tweet" in dataset.features:
            dataset = dataset.rename_column("tweet", "text")
        all_datasets.append(dataset)
        dataset_names.append(dataset_name.split("/")[-1])

    return all_datasets, dataset_names
...
def main():
    ...
    if data_args.dataset_names is not None:
        # Downloading and loading a dataset from the hub.
        raw_datasets["train"] = load_multiple_datasets(
            data_args.dataset_names,
            data_args.dataset_config_names,
            split="train",
        ...
        test_datasets, test_dataset_names = load_multiple_test_datasets(
            data_args.dataset_names,
            data_args.dataset_config_names,
            cache_dir=model_args.cache_dir,
            token=model_args.token,
    ...
    if training_args.do_eval:
        ...
        for test_dataset, test_dataset_name in zip(test_datasets, test_dataset_names):
            ...
            metrics[f"eval_{test_dataset_name}_samples"] = min(max_eval_samples, len(eval_dataset))
            trainer.log_metrics(f"eval_{test_dataset_name}", metrics)
            trainer.save_metrics(f"eval_{test_dataset_name}", metrics)
